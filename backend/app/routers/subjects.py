...
def list_subjects(db: Session = Depends(get_db)):
    return db.query(models.Subject).order_by(models.Subject.name.asc()).all()
...
def create_subject(payload: schemas.SubjectCreate, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(subject)
    log_activity(db, f'Added subject "{subject.name}"', icon="graduation-cap")
    return subject


@router.patch("/{subject_id}", response_model=schemas.SubjectOut)
def update_subject(subject_id: str, payload: schemas.SubjectUpdate, db: Session = Depends(get_db)):
    subject = db.query(models.Subject).filter(models.Subject.id == subject_id).first()
    if not subject:
        raise HTTPException(status_code=404, detail="Subject not found")
    ...
    db.commit()
    db.refresh(subject)
    log_activity(db, f'Updated subject "{subject.name}"', icon="pencil")
    return subject
...
def delete_subject(subject_id: str, db: Session = Depends(get_db)):
    ...
    db.delete(subject)
    db.commit()
    log_activity(db, f'Deleted subject "{name}"', icon="trash-2")
    return None
...
def list_topics(subject_id: Optional[str] = None, db: Session = Depends(get_db)):
...
def create_topic(payload: schemas.TopicCreate, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(topic)
    log_activity(db, f'Added topic "{topic.title}" to {subject.name}', icon="list-tree")
    return topic
