...
export function ResourceCard({ resource, hideSubject = false }: { resource: Resource; hideSubject?: boolean }) {
  const [confirmDelete, setConfirmDelete] = useState(false);
  const deleteResource = useDeleteResource();
  const Icon = TYPE_ICON[resource.resource_type] ?? FileIcon;
  const colorClasses = SUBJECT_COLOR_CLASSES[resource.subject_color ?? "purple"] ?? SUBJECT_COLOR_CLASSES.purple;
    ...
    <>
      <motion.div layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96 }}>
        <Card className="group p-4 flex items-start gap-3">
          <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${colorClasses.bg} ${colorClasses.text}`}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-1.5 mt-1 text-[11px] text-muted-foreground flex-wrap">
              {!hideSubject && resource.subject_name && <span className={colorClasses.text}>{resource.subject_name}</span>}
